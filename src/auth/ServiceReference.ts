...
export class ServiceReference<T extends Service> {
  ...
  async get(): Promise<T> {
    ...
    if (this.config.enableCaching && this.cachedService) {
      if (this.config.enableLogging) {
        console.log(`🎯 ServiceReference: Using cached service '${this.serviceName}'`);
      }
  ...
  clearCache(): void {
    if (this.config.enableCaching) {
      ...
      if (this.config.enableLogging) {
        console.log(`🗑️ ServiceReference: Cache cleared for service '${this.serviceName}'`);
      }
    }
  }
...
export class ServiceReferenceManager {
  ...
  register<T extends Service>(serviceName: string): ServiceReference<T> {
    const reference = new ServiceReference<T>(this.context, serviceName, this.config);
